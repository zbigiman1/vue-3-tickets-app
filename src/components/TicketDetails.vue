<template>
  ...
  <div v-else class="ticket-details">
    ...
    <ul class="ticket-details-list">
      <li>
        <strong>{{ t('id') }}</strong
        >: {{ ticketsStore.currentTicket.id }}
      </li>
      <li>
        <strong>{{ t('customerName') }}</strong
        >: {{ ticketsStore.currentTicket.customerName }}
      </li>
      <li>
        <strong>{{ t('subject') }}</strong
        >: {{ ticketsStore.currentTicket.subject }}
      </li>
      <li>
        <strong>{{ t('status') }}</strong
        >:
        <span class="badge" :class="ticketsStore.currentTicket.status">{{
          t(ticketsStore.currentTicket.status)
        }}</span>
      </li>
      <li>
        <strong>{{ t('priority') }}</strong
        >: {{ t(ticketsStore.currentTicket.priority) }}
      </li>
      <li>
        <strong>{{ t('createdAt') }}</strong
        >: {{ formatDate(ticketsStore.currentTicket.createdAt) }}
      </li>
    </ul>
